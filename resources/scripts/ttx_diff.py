...
def generate_output(build_dir: Path, otl_norm_bin: Path, fontmake_ttf: Path, fontc_ttf: Path):
...
def build_crate(manifest_path: Path):
    ...
    # walk upwards looking for the `target` dir; will abort at root
    while crate_dir.parent != crate_dir:
        bin_path = crate_dir / "target" / "release" / crate_name
        if bin_path.exists():
            return bin_path
