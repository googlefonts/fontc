...
static IMPORT_RESOLUTION_TESTS: &[&str] = &[
...
fn import_resolution() {
    ...
    for path in IMPORT_RESOLUTION_TESTS {
        match test_utils::run_test(PathBuf::from(path), &glyph_map, &Default::default()) {
            Ok(_) => (),
            Err(e) => panic!("{path}: {:?}", e.reason),
        }
    }
}
...
#[test]
fn multiple_languages_are_equivalent_to_lookup_references() {
...
",
        "multiple_languages_grouped",
    );
    let expanded = compile_fea(
        "\
feature locl {
    script latn;
    language AZE exclude_dflt required;
    lookup localized_i {
        sub i by j;
    } localized_i;
    language CRT exclude_dflt required;
    lookup localized_i;
    language KAZ exclude_dflt required;
    lookup localized_i;
    language TAT exclude_dflt required;
    lookup localized_i;
    language TRK exclude_dflt required;
    lookup localized_i;
} locl;
",
        "multiple_languages_expanded",
    );
...
#[test]
fn multiple_languages_include_defaults() {
    let grouped = compile_fea(
        "\
feature locl {
    script latn;
...
",
        "multiple_languages_include_defaults_grouped",
    );
    let expanded = compile_fea(
        "\
feature locl {
...
",
        "multiple_languages_include_defaults_expanded",
    );
    let glyph_map = mini_latin_glyph_map();
...
#[test]
fn multiple_languages_do_not_leak_after_state_changes() {
    let grouped = compile_fea(
        "\
feature locl {
    script latn;
...
",
        "multiple_languages_state_changes_grouped",
    );
    let expanded = compile_fea(
        "\
feature locl {
...
",
        "multiple_languages_state_changes_expanded",
    );
    let glyph_map = mini_latin_glyph_map();
...
#[test]
fn multiple_languages_deduplicate_repeated_tags() {
    let duplicates = compile_fea(
        "\
feature locl {
    script latn;
...
",
        "multiple_languages_duplicates",
    );
    let deduplicated = compile_fea(
        "\
feature locl {
    script latn;
    language DEU FRA exclude_dflt;
...
",
        "multiple_languages_deduplicated",
    );
    let glyph_map = mini_latin_glyph_map();

    assert_eq!(
        duplicates.to_binary(&glyph_map).unwrap(),
        deduplicated.to_binary(&glyph_map).unwrap(),
    );
}
