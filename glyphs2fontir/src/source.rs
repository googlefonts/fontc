...
impl KernSide {
    ...
    fn group_prefix(&self) -> &'static str {
        match self {
            KernSide::L => "public.kern1.",
            KernSide::R => "public.kern2.",
        }
    }
}
    ...
    fn exec(&self, context: &Context) -> Result<(), WorkError> {
        ...
        font.glyphs
            ...
            .flat_map(|(glyph_name, glyph)| {
                glyph
                    .left_kern
                    .iter()
                    .map(|group| (KernSide::L, group))
                    .chain(glyph.right_kern.iter().map(|group| (KernSide::R, group)))
                    .map(|(side, group_name)| {
                        (
        ...
        font.kerning_ltr
            ...
            .filter_map(|((side1, side2), pos_adjust)| {
                let side1 = kern_participant(glyph_order, &kerning.groups, KernSide::L, side1);
                let side2 = kern_participant(glyph_order, &kerning.groups, KernSide::R, side2);
