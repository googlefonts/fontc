...
fn load_designspace(
    ...
    let designspace = match ext.to_str() {
        Some("designspace") => {
            eprintln!(
                "LOADING {designspace_or_ufo:?} exits? {}",
                designspace_or_ufo.exists()
            );
...
impl Source for DesignSpaceIrSource {
    fn new(designspace_or_ufo_file: PathBuf) -> Result<Self, Error> {
        let (designspace_dir, mut designspace) = load_designspace(&designspace_or_ufo_file)
            .map_err(|kind| {
                Error::BadSource(BadSource::new(designspace_or_ufo_file.clone(), kind))
            })?;
        ...
        let Some((default_master_idx, _)) = default_master(&designspace) else {
            return Err(Error::NoDefaultMaster(designspace_or_ufo_file.clone()));
        };
        ...
        for idx in sources_indices_default_first {
            ...
            for (glyph_name, glif_file) in glif_files(&ufo_dir, &mut layer_cache, source)? {
                ...
                if idx > 0 && !glyphs.contains_key(&glyph_name) {
        ...
        Ok(DesignSpaceIrSource {
            designspace_or_ufo: Arc::new(designspace_or_ufo_file),
...
impl Work<Context, WorkId, Error> for KerningInstanceWork {
    ...
    fn exec(&self, context: &Context) -> Result<(), Error> {
        ...
        let designspace_dir = self.designspace_dir.parent().unwrap();
        let static_metadata = context.static_metadata.get();
        let glyph_order = context.glyph_order.get();
