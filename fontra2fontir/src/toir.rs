...
pub(crate) fn to_ir_static_metadata(
    ...
    let mut stat_axes = std::mem::take(&mut static_metadata.misc.stat_axes);
    for stat_axis in stat_axes.iter_mut() {
        stat_axis.labels = to_ir_axis_value_labels(font_data, stat_axis.tag);
    }
    static_metadata.set_stat(stat_axes, font_data.axes.elided_fallback_name.clone());
    Ok(static_metadata)
}
...
fn to_ir_axis_value_labels(font_data: &Font, tag: Tag) -> Vec<IrAxisValueLabel> {
