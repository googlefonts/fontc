...
_PROD_NAME_PATTERNS = (
    ...
    ProdNamePattern(
        "uni-prefix",
        lambda gi: gi.has_codepoint() and gi.production_name == f"uni{gi.codepoint:04X}",
        lambda gi: b"u" + gi.codepoint.to_bytes(3, "little"),
    ),
    ProdNamePattern(
        "u-prefix", 
        lambda gi: gi.has_codepoint() and gi.production_name == f"u{gi.codepoint:04X}",
...
def main():
    ...
    prod_name_to_idx_file = Path(__file__).parent.parent / "resources" / "prod_names_to_idx.dat"
    ...
    prod_names = [(gi.production_name_or_empty(), gi.production_name_file_bytes()) for (i, gi) in enumerate(glyph_infos)]
    prod_names.sort()
    write_list(prod_name_offsets_file, prod_name_data_file, [data for (_, data) in prod_names])
    prod_name_to_idx = {gi.production_name: i for (i, gi) in enumerate(glyph_infos) if gi.production_name is not None}
    with open(prod_name_to_idx_file, "wb") as f:
        for (prod_name, _) in prod_names:
            if prod_name not in prod_name_to_idx:
                continue
            writeU24(f, prod_name_to_idx[prod_name])
    ...
    # prod_names = sorted(prod_names)
    # print(f"{len(prod_names)} prod names from {len(glyph_infos)} glyph_infos")
    # write_string_list(prod_name_offsets_file, prod_name_data_file, prod_names)
    # with open(prod_name_to_idx_file, "wb") as f:
