...
use fontdrasil::{
    orchestration::{Access, AllOrOne, Identifier, IdentifierDiscriminant},
    types::GlyphName,
};
use fontir::{
...
impl<'a> Workload<'a> {
    ...
    fn update_be_glyph_work(&mut self, fe_root: &FeContext, glyph_name: GlyphName) {
        ...
        let mut deps = HashSet::from([AllOrOne::All(FeWorkIdentifier::StaticMetadata)]);
        ...
        if has_components {
            deps.insert(AllOrOne::All(FeWorkIdentifier::GlyphOrder));
        }
    ...
    fn can_run_all_or_one(&self, all_or_one: &AllOrOne<AnyWorkId>) -> bool {
        match all_or_one {
            AllOrOne::One(id) => !self.jobs_pending.contains_key(&id.clone()),
            AllOrOne::All(exemplar) => {
                self.count_pending
        ...
        match &job.read_access {
            AnyAccess::Fe(access) => match access {
                ...
                Access::Set(ids) => ids.iter().all(|id| match id {
                    AllOrOne::One(id) => self.can_run_all_or_one(&AllOrOne::One(id.clone().into())),
                    AllOrOne::All(exemplar) => {
                        self.can_run_all_or_one(&AllOrOne::All(exemplar.clone().into()))
                    }
                }),
