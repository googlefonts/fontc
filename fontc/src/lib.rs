...
use crate::timing::{Clock, FakeClock};
...
pub fn run(args: Args, timer: JobTimer<RealClock>) -> Result<(), Error> {
...
pub fn generate_font(
    ...
    _generate_font(
        ...
        flags,
        skip_features,
        JobTimer::<FakeClock>::default(),
    )
    .map(|(be_root, _timing)| be_root.font.get().get().to_vec())
}

fn _generate_font<T: Clock>(
    source: &Input,
    build_dir: &Path,
    output_file: Option<&PathBuf>,
    flags: Flags,
    skip_features: bool,
    mut timer: JobTimer<T>,
) -> Result<(BeContext, JobTimer<T>), Error> {
    let time = timer
...
mod tests {
    ...
    impl TestCompile {
        ...
        fn compile(source: &str, adjust_args: impl Fn(Args) -> Args) -> TestCompile {
            let timer = JobTimer::<RealClock>::new();
            ...
            let mut workload =
                Workload::<RealClock>::new(&source, timer, args.skip_features).unwrap();
