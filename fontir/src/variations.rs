...
impl VariationModel {
    ...
    pub fn interpolate_from_deltas<V>(
        ...
        deltas: Vec<(VariationRegion, Vec<V>)>,
    ) -> Vec<V>
    where
        V: Copy + Mul<f64, Output = V> + Add<V, Output = V>,
    {
        let scalars = deltas
            .iter()
            .map(|(region, _deltas)| region.scalar_at(location).into_inner() as f64)
            .collect::<Vec<_>>();
        let deltasets = deltas
            .into_iter()
            .map(|(_region, deltas)| deltas)
            .collect::<Vec<_>>();

        let mut result = None;
        for (deltas, scalar) in deltasets.iter().zip(scalars.iter()) {
            if *scalar == 0.0 {
                continue;
            }
...
mod tests {
    ...
    fn modeling_error_within_tolerance() {
        ...
        for i in 0..num_samples {
            ...
            let deltas_late_round = deltas_float
                .iter()
                .map(|(region, deltas)| {
                    (
                        region.clone(),
